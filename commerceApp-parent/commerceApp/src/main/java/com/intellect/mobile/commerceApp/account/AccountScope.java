...
public class AccountScope extends ScopeBase {
    ...
    private static final Key<CommerceAccount> GITHUB_ACCOUNT_KEY = Key
            .get(CommerceAccount.class);
        ...
        return new AbstractModule() {
            public void configure() {
                ...
                bind(GITHUB_ACCOUNT_KEY).toProvider(
                        AccountScope.<CommerceAccount> seededKeyProvider()).in(
                        scope);
            }
        };
    }

    private final ThreadLocal<CommerceAccount> currentAccount = new ThreadLocal<CommerceAccount>();

    private final Map<CommerceAccount, Map<Key<?>, Object>> repoScopeMaps = new ConcurrentHashMap<CommerceAccount, Map<Key<?>, Object>>();
    ...
    public void enterWith(final Account account,
        ...
        enterWith(new CommerceAccount(account, accountManager));
    }
     ...
     * @param account
     */
    public void enterWith(final CommerceAccount account) {
    ...
    protected <T> Map<Key<?>, Object> getScopedObjectMap(final Key<T> key) {
        CommerceAccount account = currentAccount.get();
        if (account == null)
